...
export default function ProfileSetting() {
    ...
    useEffect(() => {
        if (data) {
            ...
            if (!profilePhoto) setProfilePhoto(data.get_me.profile_pic)
            else if (profilePhoto.src) reset({ ...nameAndEmail, picture: null })
        }
    }, [reset, profilePhoto])
        ...
        <div className={styles.container}>
            <div>
                <div className={styles.logInWith}>
                    ...
                    <Button color="primary">Disconnect</Button>
                ...
                <div>
                    ...
                    <form onSubmit={passwordHandleSubmit(onPasswordChangeSubmit)}>
                        ...
                        <div className={formStyles.inputDiv}>
                            ...
                            {passwordErrors.password && <p className={formStyles.error}>{passwordErrors.password.message} </p>}
                        </div>

                        <div className={formStyles.inputDiv}>
                            <label htmlFor="confirmPassword">Confirm password</label>
                            <input {...passwordRegister('confirmPassword')} name="confirmPassword" type="password" />
                ...
                <div className={styles.information}>
                    ...
                    <div className={styles.userPhoto}>
                        ...
                        {profilePhoto && <Image src={profilePhoto} alt="user" width={70} height={70} />}
                        <label className={styles.file} htmlFor="photo">
                            Choose new image
                        </label>
                        <input {...emailAndNameRegister('picture')} onChange={handlePicture} style={{ display: 'none' }} type="file" name="photo" id="photo" />
                        <button onClick={handleImageRemove} className={styles.removeBtn}>
                            Remove Image
                        </button>
                    </div>
                    ...
                    <form onSubmit={emailAndNameHandleSubmit(onEmailAndNameChangSubmit)}>
                        <div className={formStyles.inputDiv}>
                            ...
                            {emailAndNameErrors.name && <p className={formStyles.error}>{emailAndNameErrors.name.message} </p>}
                        </div>

                        <div className={formStyles.inputDiv}>
                            <label onChange={(e) => console.log(e)} htmlFor="email">
                                Email
                            </label>
                            <input {...emailAndNameRegister('email')} name="email" type="text" />
                            {emailAndNameErrors.email && <p className={formStyles.error}>{emailAndNameErrors.email.message} </p>}
                        </div>
