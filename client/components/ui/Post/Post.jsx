...
import { IoBookmarkOutline, IoBookmark, IoHeart, IoChatboxOutline, IoEllipsisVerticalSharp, IoHeartOutline, IoShareSocialOutline, IoEllipsisHorizontalSharp } from "react-icons/io5"
...
export default function Post({ image, comment, info }) {
  ...
  return (
    <div className={styles.post}>
      <div className={styles.info} style={{ justifyContent: comment ? "space-between" : "space-around" }}>
        <div>
        ...
        <button className="icon">{comment ? <IoEllipsisHorizontalSharp size={25} color="#c5c7c9" /> : <IoEllipsisVerticalSharp size={25} color="#c5c7c9" />}</button>
      </div>
      <div className={styles.content} style={{ margin: comment ? "unset" : "0 auto 1rem" }}>
        <p>{content}</p>
      </div>
      <div className={styles.icons} style={{ justifyContent: comment ? "space-around" : "space-evenly" }}>
